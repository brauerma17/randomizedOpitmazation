...
def maxKColor(edges, nodes, colors):
    ...
    problem = mlrose.DiscreteOpt(length = nodes, fitness_fn = fitness, maximize = True, max_val = colors)
    t0 = time()
...
def KQueens(numQueens, sizeOrIterations):
    ...
    problem = mlrose.DiscreteOpt(length = numQueens, fitness_fn = fitness, maximize = True, max_val = numQueens)
    rhcfitnessMatrix = []
    safitnessMatrix = []
    genalgfitnessMatrix = []
    mimicfitnessMatrix = []
